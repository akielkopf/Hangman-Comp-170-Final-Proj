...
namespace IntroCS
{
  ...
  public class UI
  {
    ...
    /// compare integer strings: any lengths, but both positive or both negative
    ...
    /// Prompt the user to enter an integer until the response is legal. Return
    /// the result as in int.
    ...
    /// Prompt the user until a keyboard entry is an int in the range [lowLim,
    /// highLim]. Then return the int value in range. Append the range to the prompt.
    ...
    // helper string testing functions for same length, lexicographical
    // comparison works
  }
}